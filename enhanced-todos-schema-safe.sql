-- Enhanced AGGRANDIZE Team Todos Database Schema - Safe Version
-- This version handles existing policies and tables gracefully

-- First, add new columns to existing todos table (safe with IF NOT EXISTS)
ALTER TABLE todos 
...
-- Create todo_assignee_status table (safe with IF NOT EXISTS)
...
-- Enhanced todo_comments table (safe with IF NOT EXISTS)
...
-- Drop and recreate todo_attachments table with better schema
...
-- Create activity log (safe with IF NOT EXISTS)
...
-- Create notification table (safe with IF NOT EXISTS)
...
-- Add indexes for better performance (safe with IF NOT EXISTS)
...
DO $$
BEGIN
    -- Drop existing policies for todo_assignee_status
    DROP POLICY IF EXISTS "Users can view assignee status" ON todo_assignee_status;
    DROP POLICY IF EXISTS "Users can update their own assignee status" ON todo_assignee_status;
    DROP POLICY IF EXISTS "System can manage assignee status" ON todo_assignee_status;
    
    -- Drop existing policies for todo_activity_log
    DROP POLICY IF EXISTS "Users can view activity log" ON todo_activity_log;
    DROP POLICY IF EXISTS "System can create activity log" ON todo_activity_log;
    
    -- Drop existing policies for todo_notifications
    DROP POLICY IF EXISTS "Users can view own notifications" ON todo_notifications;
    DROP POLICY IF EXISTS "System can manage notifications" ON todo_notifications;
    
    -- Drop existing policies for todo_attachments
...
-- Create new RLS policies for assignee status
...
-- Create new RLS policies for activity log
...
-- Create new RLS policies for attachments
...
-- Sample data for testing enhanced features (only insert if no existing todos with these titles)
INSERT INTO todos (title, description, created_by, assigned_to_array, category, priority, status, is_team_todo, due_date) 
SELECT 'Design New Dashboard Layout', 'Create mockups and implement responsive design for the new dashboard', 'dhana@aggrandizedigital.com', 
       ARRAY['veera@aggrandizedigital.com', 'saravana@aggrandizedigital.com'], 'feature', 'high', 'in_progress', true, now() + interval '1 week'
WHERE NOT EXISTS (SELECT 1 FROM todos WHERE title = 'Design New Dashboard Layout');

INSERT INTO todos (title, description, created_by, assigned_to_array, category, priority, status, is_team_todo, due_date) 
SELECT 'Client Onboarding Process', 'Streamline the client onboarding workflow with automated emails', 'dhana@aggrandizedigital.com',
       ARRAY['saran@aggrandizedigital.com', 'abbas@aggrandizedigital.com'], 'work', 'medium', 'todo', true, now() + interval '2 weeks'
WHERE NOT EXISTS (SELECT 1 FROM todos WHERE title = 'Client Onboarding Process');

-- Sample comments with mentions (only insert if tasks exist)
INSERT INTO todo_comments (todo_id, comment_by, comment, mentions) 
SELECT id, 'dhana@aggrandizedigital.com', 
       'Starting work on this. @veera@aggrandizedigital.com can you review the wireframes?', 
       ARRAY['veera@aggrandizedigital.com']
FROM todos WHERE title = 'Design New Dashboard Layout' 
AND NOT EXISTS (SELECT 1 FROM todo_comments WHERE todo_id = (SELECT id FROM todos WHERE title = 'Design New Dashboard Layout' LIMIT 1) LIMIT 1);

INSERT INTO todo_comments (todo_id, comment_by, comment, mentions) 
SELECT id, 'veera@aggrandizedigital.com', 
       'Looks good! I''ll start on the CSS components.', 
       ARRAY[]::TEXT[]
FROM todos WHERE title = 'Design New Dashboard Layout'
AND NOT EXISTS (SELECT 1 FROM todo_comments WHERE comment_by = 'veera@aggrandizedigital.com' AND todo_id = (SELECT id FROM todos WHERE title = 'Design New Dashboard Layout' LIMIT 1));

-- Success message
DO $$
BEGIN
    RAISE NOTICE 'Enhanced todos schema has been successfully applied!';
END $$;